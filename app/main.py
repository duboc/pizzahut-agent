...
APP_NAME = "Pizza Hut - Assistente Pizza Hut"
...
@app.get("/debug/pedidos")
async def debug_pedidos():
    """Debug endpoint to check current orders"""
    from food.tools.tools import pedidos_em_processo
    return {"pedidos_em_processo": pedidos_em_processo}


@app.get("/api/pedido/{session_id}")
async def get_pedido(session_id: str):
...
async def debug_menu():
    """Debug endpoint to check Pizza Hut menu"""
    from food.tools.tools import PIZZAHUT_MENU
    return {"menu_pizzahut": PIZZAHUT_MENU}
